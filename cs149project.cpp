...
string trim(string str) {
    ...
    size_t firstNonWhiteSpace = str.find_first_not_of(whiteSpaces);
    str.erase(0, firstNonWhiteSpace);

    size_t lastNonWhiteSpace = str.find_last_not_of(whiteSpaces);
            ...
            switch (instruction.operation) {
                ...
                case 'R': // String argument
                    ...
                    if (instruction.stringArg.size() == 0) {
                        std::cout << filename << ":" << lineNum << " - Missing string argument, " 
                             << endl;
                        file.close();
                        return false;
                    }
                    break;
                default:
                    std::cout << filename << ":" << lineNum << " - Invalid operation, "
...
void schedule()
{
    ...
    if (runningState != -1) /* if runningState is not empty */
    {
        return;
    }
...
void replace(string &argument)
{
    ...
    if (!createProgram(argument, *cpu.pProgram)) {
        std::cout << "FATAL ERROR: Failed to run createProgram; File could not be opened or does not exist" << endl;
        return;
    }
...
void quantum()
{
    ...
    std::cout << "In quantum";
    if (runningState == -1) {
        std::cout << "No processes are running" << endl;
        ++timestamp;
        return;
    }
    ...
    if (cpu.programCounter < cpu.pProgram->size()) {
        ...
        std::cout << "End of program reached without E operation" << endl;
    ...
    switch (instruction.operation) {
        case 'S':
            ...
            std::cout << "instruction S " << instruction.intArg << endl;
            break;
        case 'A':
            add(instruction.intArg);
            std::cout << "instruction A " << instruction.intArg << endl;
            break;
        case 'D':
...
void print() {
    std::cout << "Print command is not implemented until iLab3" << endl;
}
...
int runProcessManager(int fileDescriptor) {
    ...
    do {
        // Read a command chracter from the pipe.
        ...
        switch (ch) {
            ...
            case 'U':
                unblock();
                std::cout << "You entered U" << endl;
                break;
            case 'P':
                print();
                std::cout << "You entered P" << endl;
                break;
            default:
                std::cout << "You entered an invalid character!" << endl;
        }
    } while (ch != 'T');
    return EXIT_SUCCESS;
}
...
int main(int argc, char *argv[]) {
    ...
    // TODO: Create a pipe
    if (pipe(pipeDescriptors) == -1) { return -1; } /* pipe failed */
    ...
    if (processMgrPid == 0)
    {
        ...
        // Close the unused read pipe for the commander process.
        close(pipeDescriptors[0]);

        // Loop until a 'T' is written or until the pipe is broken.
        do {
            std::cout << "Enter Q, P, U, or T" << endl;
            std::cout << "$ ";
            cin >> ch;
